...
async function checkTwilio(sid, token) {
    try {
        ...
        if (res.ok)
            return { name: 'Twilio auth', ok: true, detail: `account ${sid.slice(0, 4)}***${sid.slice(-4)}` };
...
async function checkOpenAI(key) {
    try {
        ...
        if (res.ok)
            return { name: 'OpenAI API reachable', ok: true };
...
async function checkElevenLabs(key) {
    try {
        ...
        if (res.ok)
            return { name: 'ElevenLabs API reachable', ok: true };
...
export async function runDoctor() {
    ...
    try {
        ...
        if (creds.OPENAI_API_KEY)
            checks.push(await checkOpenAI(creds.OPENAI_API_KEY));
        if (creds.ELEVENLABS_API_KEY)
            checks.push(await checkElevenLabs(creds.ELEVENLABS_API_KEY));
        if (creds.DEEPGRAM_API_KEY)
            checks.push(await checkDeepgram(creds.DEEPGRAM_API_KEY));
    }
    catch (err) {
