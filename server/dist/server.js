...
import { buildPatter } from './patter.js';
...
const TOOLS = [
    {
        ...
        needsPatter: true,
        validate: (a) => { validateMakeCallInput(a); },
        handler: (ctx, a) => makeCall(ctx, a),
    },
    {
        ...
        needsPatter: true,
        validate: (a) => { validateCallThirdPartyInput(a); },
        handler: (ctx, a) => callThirdParty(ctx, a),
    },
    {
        name: 'get_calls',
        ...
        needsPatter: false,
        validate: (a) => { validateGetCallsInput(a); },
        handler: (_ctx, a) => getCalls(a),
    },
    {
        name: 'get_transcript',
        ...
        needsPatter: false,
        validate: (a) => { validateGetTranscriptInput(a); },
        handler: (_ctx, a) => getTranscript(a),
    },
];
export async function startMcpServer() {
    const server = new Server({ name: 'claude-call', version: '0.2.1' }, { capabilities: { tools: {} } });
    let ctxPromise;
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        try {
            tool.validate(args);
            const ctx = tool.needsPatter ? await getCtx() : undefined;
