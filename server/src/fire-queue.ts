...
interface FireRequest {
  tool: 'make_call' | 'call_third_party';
...
async function processOne(file: string, ctx: PatterContext): Promise<void> {
  const path = join(queueDir(), file);
  let req: FireRequest;
  try {
    req = JSON.parse(readFileSync(path, 'utf8')) as FireRequest;
  } catch (err) {
...
export function startFireQueueWatcher(getCtx: () => Promise<PatterContext>): void {
  ensureDir();
  let busy = false;
...
export function enqueueFire(req: FireRequest, file?: string): string {
  ensureDir();
