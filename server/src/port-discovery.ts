...
interface ProbeResult {
  usable: boolean;
  /** Reason for skipping; populated only when usable is false. Useful in logs. */
  reason?: 'ipv4_in_use' | 'ipv6_listener_present';
}
...
async function probePort(port: number): Promise<ProbeResult> {
  ...
  if (!ipv4Bindable) return { usable: false, reason: 'ipv4_in_use' };

  const ipv6Empty = await ipv6LoopbackHasNoListener(port);
  if (!ipv6Empty) return { usable: false, reason: 'ipv6_listener_present' };

  return { usable: true };
}

function canBindIpv4(port: number): Promise<boolean> {
...
function ipv6LoopbackHasNoListener(port: number): Promise<boolean> {
  return new Promise((resolve) => {
    const socket = new net.Socket();
    let settled = false;
    const settle = (empty: boolean): void => {
      if (settled) return;
      settled = true;
...
export interface FindFreePortOptions {
  ...
  /** Optional sink for per-port skip diagnostics. Defaults to a no-op. */
  onSkip?: (port: number, reason: NonNullable<ProbeResult['reason']>) => void;
}
...
export async function findFreePort(opts: FindFreePortOptions = {}): Promise<number> {
  ...
  for (let port = start; port <= end; port++) {
    ...
    if (result.usable) return port;
    if (opts.onSkip && result.reason) opts.onSkip(port, result.reason);
  }
  throw new Error(
    `No usable port in [${start}, ${end}]. Every candidate is either bound on 127.0.0.1 or has an IPv6 listener that would hijack the Cloudflare tunnel.`,
  );
}
