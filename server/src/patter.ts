...
export async function ensureServing(ctx: PatterContext, target: ServingTarget): Promise<void> {
  ...
  // docs/postmortems/2026-05-08-ipv6-localhost-tunnel-hijack.md for the full
  ...
  const port = await findFreePort({
    onSkip: (skippedPort, reason) =>
      void logEvent({ event: 'port_skipped', port: skippedPort, reason }),
  });
