...
**Tech Stack:** Node 20+, TypeScript 5, ESM, `@modelcontextprotocol/sdk@^1.29`, `getpatter@^0.5.4`, `tsup` (build), `vitest` (tests), `cloudflared` (optional, transitive via getpatter).
...
- `server/.gitignore` — `node_modules/`, but **not** `dist/` (committed)
...
- [ ] **Step 5: Write placeholder `server/src/index.ts`**
...
- [ ] **Step 6: Install deps and verify build pipeline**

Run from repo root (note quoted brackets):
```bash
cd "$REPO/server" && npm install
```
Expected: installs without warnings other than peer-dep notices.
...
```bash
cd "$REPO/server" && npx vitest run --reporter=verbose
```
Expected: `No test files found` (we haven't written any yet) — exits 0.

- [ ] **Step 7: Commit**

```bash
cd "$REPO" && git add server/ && git commit -m "feat(server): scaffold TypeScript MCP server sub-package"
```
...
describe('redactPhone', () => {
  it('keeps +<2-digit-country-code> prefix and last-4, masks middle', () => {
    expect(redactPhone('+393331234567')).toBe('+39******4567');
    expect(redactPhone('+15551234567')).toBe('+15*****4567');
  });
  ...
  it('preserves leading + and digits in the first 3 positions', () => {
    expect(redactPhone('+44 7700 900123')).toMatch(/^\+44.*0123$/);
  });
});
```
...
```bash
cd "$REPO/server" && npx vitest run tests/unit/redact.test.ts
```
...
describe('logEvent', () => {
  ...
  it('redacts phone-number fields automatically', async () => {
    ...
    const line = readFileSync(logPath, 'utf8').trim();
    const parsed = JSON.parse(line);
    expect(parsed.to).toBe('+39******4567');
    expect(parsed.from).toBe('+15*****4567');
  });
...
```bash
cd "$REPO/server" && npx vitest run tests/unit/log.test.ts
```
Expected: FAIL — module not found.
...
describe('loadCredentials', () => {
  it('loads valid credentials and returns typed object', async () => {
    ...
    expect(creds.TWILIO_ACCOUNT_SID).toBe('AC00000000000000000000000000000000');
    expect(creds.VOICE_ENGINE).toBe('openai_realtime');
    expect(creds.OPENAI_API_KEY).toBe('sk-test');
  });

  it('throws credentials_missing when file is absent', async () => {
    await expect(loadCredentials(credPath)).rejects.toMatchObject({ code: 'credentials_missing' });
  });

  it('throws credentials_invalid when required key is missing', async () => {
    writeFileSync(credPath, 'TWILIO_ACCOUNT_SID=AC1\n', 'utf8');
    chmodSync(credPath, 0o600);
    await expect(loadCredentials(credPath)).rejects.toBeInstanceOf(CredentialsError);
  });
  ...
  it('throws credentials_invalid when phone number is not E.164', async () => {
    ...
    chmodSync(credPath, 0o600);
    await expect(loadCredentials(credPath)).rejects.toMatchObject({ code: 'credentials_invalid' });
  });
...
```bash
cd "$REPO/server" && npx vitest run tests/unit/credentials.test.ts
```
Expected: FAIL — module not found.
...
describe('store', () => {
  it('appendCall + listCalls round-trips a record', async () => {
    ...
    expect(calls[0].call_id).toBe('CA1');
    expect(calls[0].duration_seconds).toBe(30);
  });
  ...
  it('listCalls returns most-recent-first by started_at', async () => {
    ...
    const calls = await listCalls({}, path);
    expect(calls[0].call_id).toBe('CA2');
    expect(calls[1].call_id).toBe('CA1');
  });
...
```bash
cd "$REPO/server" && npx vitest run tests/unit/store.test.ts
```
Expected: FAIL — module not found.
...
describe('validateMakeCallInput', () => {
  it('rejects non-E.164 numbers', () => {
    expect(() => validateMakeCallInput({ to: '5551234567', system_prompt: 'hi' })).toThrow(/E\.164/);
  });

  it('rejects missing system_prompt', () => {
    expect(() => validateMakeCallInput({ to: '+15551234567' } as never)).toThrow(/system_prompt/);
  });

  it('rejects oversize system_prompt', () => {
    const big = 'x'.repeat(8001);
    expect(() => validateMakeCallInput({ to: '+15551234567', system_prompt: big })).toThrow(/8000/);
  });

  it('accepts valid input', () => {
    const v = validateMakeCallInput({ to: '+15551234567', system_prompt: 'Be helpful.', first_message: 'Hi' });
    expect(v.to).toBe('+15551234567');
    expect(v.first_message).toBe('Hi');
  });
});
```
...
```bash
cd "$REPO/server" && npx vitest run tests/unit/tools.test.ts
```
Expected: FAIL — module not found.
...
describe('validateCallThirdPartyInput', () => {
  it('rejects empty objective', () => {
    expect(() => validateCallThirdPartyInput({ to: '+15551234567', objective: '' })).toThrow(/objective/);
  });

  it('rejects oversize objective', () => {
    expect(() => validateCallThirdPartyInput({ to: '+15551234567', objective: 'x'.repeat(2001) })).toThrow(/2000/);
  });

  it('clamps max_turns to [1, 30]', () => {
    expect(validateCallThirdPartyInput({ to: '+15551234567', objective: 'book', max_turns: 100 }).max_turns).toBe(30);
    expect(validateCallThirdPartyInput({ to: '+15551234567', objective: 'book', max_turns: 0 }).max_turns).toBe(1);
  });
...
describe('validateGetCallsInput', () => {
  it('defaults limit to 20', () => {
    expect(validateGetCallsInput({}).limit).toBe(20);
  });

  it('clamps limit to [1, 200]', () => {
    expect(validateGetCallsInput({ limit: 1000 }).limit).toBe(200);
    expect(validateGetCallsInput({ limit: 0 }).limit).toBe(1);
  });

  it('rejects malformed since timestamp', () => {
    expect(() => validateGetCallsInput({ since: 'yesterday' })).toThrow(/ISO8601/);
  });

  it('accepts valid since timestamp', () => {
    const v = validateGetCallsInput({ since: '2026-04-28T10:00:00Z' });
    expect(v.since).toBe('2026-04-28T10:00:00Z');
  });
});
```
...
describe('validateGetTranscriptInput', () => {
  it('rejects missing call_id', () => {
    expect(() => validateGetTranscriptInput({} as never)).toThrow(/call_id/);
  });

  it('rejects empty call_id', () => {
    expect(() => validateGetTranscriptInput({ call_id: '' })).toThrow(/call_id/);
  });

  it('accepts non-empty call_id', () => {
    expect(validateGetTranscriptInput({ call_id: 'CA1' }).call_id).toBe('CA1');
  });
});
```
...
describe('MCP server (stdio)', () => {
  it('responds to initialize', async () => {
    ...
    expect(res).toMatchObject({ id: 1, result: { protocolVersion: expect.any(String) } });
  });

  it('lists 4 tools', async () => {
    const res = await send({ jsonrpc: '2.0', id: 2, method: 'tools/list', params: {} }) as { result: { tools: { name: string }[] } };
    const names = res.result.tools.map((t) => t.name);
    expect(names).toEqual(expect.arrayContaining(['make_call', 'call_third_party', 'get_calls', 'get_transcript']));
  });
  ...
  it('returns credentials_missing in degraded mode for make_call', async () => {
    ...
    expect(parsed.error).toBe('credentials_missing');
    expect(parsed.action).toMatch(/setup/);
  });
});
```
...
```bash
cd "$REPO/server" && npm run build && npx vitest run tests/integration/mcp.test.ts
```
Expected: 3/3 pass.
...
describe('doctor', () => {
  it('exits 1 when credentials are missing', () => {
    ...
    expect(res.out).toMatch(/credentials/i);
  });
...
```bash
cd "$REPO/server" && npm run build && npx vitest run tests/integration/doctor.test.ts
```
Expected: 2/2 pass.
