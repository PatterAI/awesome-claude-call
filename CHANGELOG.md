...
### Fixed
- **Outbound calls connected on the carrier but the user heard silence; `make_call` then hung for the full 5-minute timeout and returned `call_failed: exceeded 300000ms timeout`.** Symptom only manifested on hosts running a Docker container that mapped `[::]:8000` or `[::]:8001` (e.g. the official `patter-dashboard`). Root cause: `getpatter@0.5.4` starts the Cloudflare quick-tunnel with the literal target string `http://localhost:<port>` and macOS resolves `localhost` to `::1` before `127.0.0.1`. The Docker IPv6 listener silently absorbed all tunnel traffic; the Patter server on `127.0.0.1:8000` saw nothing; Twilio's status callback never got a response; `metricsStore` never published `call_end`; `awaitCallEnd` timed out. v0.2.3's `clearStalePatterWebhookUrl` workaround actually exposed this bug by removing the noisier `Cannot use both tunnel: true and webhookUrl` error that had been masking it. New module `server/src/port-discovery.ts` probes a range (default `[8002, 8099]`) and picks the first port that is *both* (a) bindable on `127.0.0.1` and (b) silent on `::1`. The dual check is load-bearing — probing only IPv4 would miss the Docker hijack; probing only IPv6 would miss a stray IPv4 conflict. `ensureServing` calls it once per `serve()` and threads the chosen port through `ServeOptions.port`. New unit tests in `server/tests/unit/port-discovery.test.ts` cover the IPv4-bind / IPv6-listener matrix using real sockets in an out-of-band probe range. Full investigation in `docs/postmortems/2026-05-08-ipv6-localhost-tunnel-hijack.md`.

### Added
- `port_chosen` and `port_skipped` log events in `~/.claude-call/log.ndjson` so future port-collision incidents are diagnosable from a single `tail`.
